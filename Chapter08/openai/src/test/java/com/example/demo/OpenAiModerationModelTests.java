    ...
    public void testOpenAiModerationModel() {
        ...
        for(ModerationResult result:moderation.getResults()){
            ...
            log.info("증오 또는 혐오: {} {}",
                    ...
                    scores.getHate());
            log.info("괴롭힘 또는 희롱: {} {}",
                    categories.isHarassment(),
                    scores.getHarassment());
            log.info("자해: {} {}",
                    categories.isSelfHarm(),
                    scores.getSelfHarm());
            log.info("미성년자와 관련된 성적인 콘텐츠: {} {}",
                    categories.isSexualMinors(),
                    scores.getSexualMinors());
            log.info("위협적인 증오 발언: {} {}",
                    categories.isHateThreatening(),
                    scores.getHateThreatening());
            log.info("폭력적이고 잔인한 묘사: {} {}",
                    categories.isViolenceGraphic(),
                    scores.getViolenceGraphic());
            log.info("자해 의도: {} {}",
                    categories.isSelfHarmIntent(),
                    scores.getSelfHarmIntent());
            log.info("자해 방법에 대한 지침: {} {}",
